...
test("After filling Data in the Comment Form", async() => {
    render(<CommentForm /> );
    const commentInput = screen.getByRole("textbox");
    const checkboxElem = screen.getByLabelText("i agree to terms and conditions",  {exact: false});
    const submitElem = screen.getByRole("button", {exact: false});
    ...
    await userEvent.type(commentInput, "Hello World"); // this is for userEvent testing
    // for userEvent we need to use async await
